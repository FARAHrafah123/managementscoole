...
if (isset($_POST['fname']) &&
    ...
    if (empty($fname)) {
        ...
        $sql = "INSERT INTO registrar_office(username, password, fname, lname, address, date_of_birth, phone_number, qualification, gender, email_address)
                 VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
           $stmt = $conn->prepare($sql);
        $stmt->execute([$uname, $pass, $fname, $lname, $address, $date_of_birth, $phone_number, $qualification, $gender, $email_address]);
        $sm = "New registrar-office registered successfully";
